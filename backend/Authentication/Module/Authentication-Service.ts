...
type SafeUser = {
    _id: string;
    email: string;
    role: string;        // or UserRole if you import it
...
export class AuthService {
    ...
    // in AuthService.ts (top)


// in AuthService.ts (private helper)
    private toSafeUser(doc: any): SafeUser {
        const obj = (typeof doc.toObject === 'function' ? doc.toObject() : doc) as any;
        return {
            _id: String(obj._id),
    ...
    async register(registerDto: any) {
        ...
        const newUser = await this.userService.create({
            ...registerDto,
            // create() already sets passwordHash, role, and initializes flags in your service
        });
        ...
        try {
            await this.mail.send({
                to: newUser.email!, // email is present per schema
    ...
    async validateUser(email: string, plainPassword: string) {
        ...
        if (!user) {
            await this.audit.log('LOGIN_FAILED', undefined, { email, reason: 'UNKNOWN_EMAIL' });
            throw new UnauthorizedException('Invalid credentials');
        }

        const storedHash = (user as any).passwordHash as string | undefined;
        if (!storedHash) {
            await this.audit.log('LOGIN_FAILED', String(user._id), { email, reason: 'NO_PASSWORD_HASH' });
            throw new UnauthorizedException('Invalid credentials');
        }

        const ok = await bcrypt.compare(plainPassword, storedHash);
        if (!ok) {
            await this.audit.log('LOGIN_FAILED', String(user._id), { email, reason: 'BAD_PASSWORD' });
            throw new UnauthorizedException('Invalid credentials');
        }
    ...
    private async issueTempMfaToken(userId: string, email: string, role: string) {
        return this.jwtService.sign(
            { sub: userId, email, role, mfa: true },
            { expiresIn: '5m' },
        );
    }

    async login(email: string, plainPassword: string) {
        const user = await this.validateUser(email, plainPassword); // SafeUser
        ...
        await this.audit.log('LOGIN_SUCCESS', user._id, { email: user.email, role: user.role });
        return { access_token, refresh_token, user };
    }

    async logout(rawBearerToken: string): Promise<{ message: string }> {
        const decoded = this.jwtService.decode(rawBearerToken);
        if (!decoded || typeof decoded !== 'object' || !('exp' in decoded)) {
            throw new ForbiddenException('Invalid token');
        }
        const exp = (decoded as any).exp as number;
        await this.blacklistModel.create({ token: rawBearerToken, expiresAt: new Date(exp * 1000) });
    ...
    async getUserProfile(userId: string) {
        // use your public projector or the small findById wrapper we added above
    ...
    async verifyOTP(email: string, otpCode: string) {
        ...
        if (user.otpCode !== otpCode || !user.otpExpiresAt || new Date() > user.otpExpiresAt) {
    ...
    async verifyLoginWithMfa(userId: string, input: { token?: string; backup?: string }) {
        const user = await this.userService.findByIdSelectSecret(userId);
        if (!user?.mfaSecret || !user.mfaEnabled) throw new UnauthorizedException('mfa not enabled');

        let ok = false;
        if (input.token) {
            ok = speakeasy.totp.verify({ secret: user.mfaSecret, encoding: 'base32', token: input.token, window: 1 });
    ...
    async disableMfa(userId: string) {
        await this.userService.updateUserInternal(userId, { mfaEnabled: false, mfaSecret: null, mfaBackupCodes: [] });
    ...
    // Convenience wrappers used by controller
    async sendOTP(email: string) {
        return this.sendOtpGeneric(email, 'verification', false);
    }
    async resendOTP(email: string) {
        return this.sendOtpGeneric(email, 'verification', true);
    }
    async forgotPassword(email: string) {
        return this.sendOtpGeneric(email, 'password-reset', false);
    }
    ...
    private async sendOtpGeneric(
        ...
        const last = user.otpExpiresAt || null;
        if (rateLimit && last && Date.now() - last.getTime() < 2 * 60 * 1000) {
        ...
        await this.userService.updateUserInternal(user._id.toString(), { otpCode, otpExpiresAt });

        let subject = 'Your OTP Code';
        let text = `Your OTP Code is: ${otpCode}`;
        if (purpose === 'verification') {
            subject = 'Verify Your Email Address';
            text = `Your email verification code is: ${otpCode}. It expires in 10 minutes.`;
...
//
//
//     // private signAccess(payloadBase: Omit<JwtPayload, 'jti' | 'mfa'>) {
//     //     return this.jwtService.signAsync(payloadBase, { expiresIn: '15m' }); // shorter is better
//     // }
//     // private signRefresh(payloadBase: Omit<JwtPayload, 'mfa'>) {
//     //     const jti = randomUUID();
//     //     return this.jwtService.signAsync({ ...payloadBase, jti }, { expiresIn: '7d' });
//     // }
//
//
...
//     //     }
//     //
//     //     const base = { sub: String(user._id), email: user.email, role: user.role as any };
//     //     const access_token  = await this.signAccess(base);
//     //     const refresh_token = await this.signRefresh(base);
//     //
//     //     await this.audit.log('LOGIN SUCCESSFUL', String(user._id), { email: user.email, role: user.role });
//     //     return { access_token, refresh_token, user };
//     // }
//
...
//     }
//
//     // async refreshViaGuard(payload: JwtPayload) {
//     //     // payload contains sub,email,role,jti from JwtRefreshStrategy
//     //     // blacklist old jti (rotation)
//     //     if (payload.jti) await this.blacklistJti(payload.jti, /*ttl*/ 7 * 24 * 3600);
//     //
...
//
//
//     // AuthService.ts (blacklist helpers)
...
// //         });
// //     }
// //     async isJtiBlacklisted(jti: string) {
// //         const doc = await this.blacklistModel.findOne({ jti }).select('_id').lean();
// //         return !!doc;
// //     }
// //
// // // OPTIONAL: if you still want to blacklist access tokens by raw string:
// //     async isAccessTokenBlacklisted(token: string) {
// //         const doc = await this.blacklistModel.findOne({ token }).select('_id').lean();
// //         return !!doc;
// //     }
//
...
//     }
//
// //     const last = user.otpLastSentAt;
// //     if (rateLimit && last && (Date.now() - last.getTime()) < 120_000) {
// //     throw new BadRequestException('Please wait before requesting a new OTP');
// // }
// //
// // await this.userService.updateUser(user._id.toString(), {
