...
import {AuditEvent, AuditLog, AuditLogDocument} from "../../Audit-Log/Model/Audit-Log";
import {
    Notification,
...
import {TokenBlacklistService} from "../../Authentication/Token/BlackList-Token.Service";
...
export class AdminService {
    ...
    async createUserByAdmin(dto: CreateUserDto, adminId: string) {
        ...
        await this.audit.record(AuditEvent.ADMIN_CREATED_USER, adminId, {
    ...
    async metrics() {
        ...
        const [
            ...
            this.auditModel.countDocuments({ event: AuditEvent.LOGIN_SUCCESS, timestamp: { $gte: dayAgo } }).exec(),
            this.auditModel.countDocuments({ event: AuditEvent.PASSWORD_RESET_REQUESTED, timestamp: { $gte: dayAgo } }).exec(),
    ...
    async securityOverview(params?: { limit?: number; from?: string; to?: string }) {
        ...
        const q: FilterQuery<AuditLogDocument> = {
            ...
            event: {$in: [AuditEvent.LOGIN_FAILED, AuditEvent.UNAUTHORIZED_ACCESS, AuditEvent.TOKEN_BLACKLISTED, AuditEvent.RBAC_DENIED, AuditEvent.ADMIN_FORCE_LOGOUT, AuditEvent.ADMIN_USER_LOCK, AuditEvent, AuditEvent.ADMIN_USER_UNLOCK, AuditEvent.MFA_ENABLED, AuditEvent.MFA_DISABLED]},
        };
