...
export class BackupService {
    ...
    private COMPRESS = String(process.env.BACKUP_COMPRESS || 'false').toLowerCase() === 'true';
    ...
    private redactUser(u: any) {
        const { password, passwordHash, mfaSecret, mfaBackupCodes, otpCode, otpExpiresAt, ...safe } = u || {};
        return safe;
    }

    private async dumpUsers() {
        const docs = await this.userModel.find().lean().exec();
        return docs.map((d: any) => this.redactUser(d));
    }
    ...
    async runBackup(dto: RunBackupDTO) {
        ...
        const results: Array<{ type: string; file: string; count: number; sizeBytes?: number; checksumSha256?: string; compressed?: boolean }> = [];

        for (const t of types) {
            let data: any[] = [];
            if (t === BackupDataType.USERS) data = await this.dumpUsers();
            else if (t === BackupDataType.COURSES) { /* plug course dump when available */ }
            else if (t === BackupDataType.PERFORMANCES) { /* plug performances dump when available */ }
            else continue;

            const base = `backup-${t}-${ts}.json`;
            const jsonPath = path.join(this.BACKUP_DIR, base);
            const payload = JSON.stringify({ type: t, exportedAt: new Date(), items: data }, null, 2);

            fs.writeFileSync(jsonPath, payload, 'utf8');

            let storageLink = `file://${jsonPath}`;
            let sizeBytes = fs.statSync(jsonPath).size;
            ...
            if (this.COMPRESS) {
                ...
                await pipeline(Readable.from(payload), gzip, tap, fs.createWriteStream(gzPath));
                checksumSha256 = hasher.digest('hex');
                storageLink = `file://${gzPath}`;
                sizeBytes = fs.statSync(gzPath).size;
                compressed = true;
                // optionally remove original json
            ...
            results.push({ type: String(t), file: storageLink, count: data.length, sizeBytes, checksumSha256, compressed });
        }
    ...
    /** Restore stub (document-only): validate link exists; actual data import is environment-specific. */
    async restore(id: string) {
        const backup = await this.getOne(id);
        const p = backup.storageLink.replace(/^file:\/\//, '');
        if (!fs.existsSync(p)) throw new NotFoundException('Backup file not found on disk');
        return { ok: true, message: 'Restore procedure is environment-specific. File is present.', storageLink: backup.storageLink };
    }
}
